use gimli::{BaseAddresses, BigEndian, CfaRule, CieOrFde, DebugFrame, EhFrame,
            FrameDescriptionEntry, LittleEndian, Reader, ReaderOffset, RegisterRule,
            UninitializedUnwindContext, UnwindOffset, UnwindSection, UnwindTable};
use std::collections::HashMap;
use std::io::Write;

use debuginfo::{Dwarf, ObjectFile};
use errors::Result;
use registers;

struct BreakpadAsciiCfiWriter<W: Write> {
    inner: W,
}

/// TODO(ja): Support architectures
impl<W: Write> BreakpadAsciiCfiWriter<W> {
    ...
    fn process_dwarf<'a, D: Dwarf<'a>>(&mut self, dwarf: &D) -> Result<()> {
        if let Some(section) = dwarf.get_section(".eh_frame") {
            if dwarf.little_endian() {
                let frame = EhFrame::new(section.data, LittleEndian);
                self.read_cfi(frame, section.offset)
            } else {
                let frame = EhFrame::new(section.data, BigEndian);
                self.read_cfi(frame, section.offset)
            }
        } else if let Some(section) = dwarf.get_section(".debug_frame") {
            if dwarf.little_endian() {
                let frame = DebugFrame::new(section.data, LittleEndian);
                self.read_cfi(frame, section.offset)
            } else {
                let frame = DebugFrame::new(section.data, BigEndian);
                self.read_cfi(frame, section.offset)
            }
        } else {
            Err("".into())
    ...
    fn read_cfi<U, R>(&mut self, frame: U, base: u64) -> Result<()>
    where
        R: Reader + Eq,
        U: UnwindSection<R>,
    {
    ...
    fn process_fde<S, R, O>(&mut self, fde: FrameDescriptionEntry<S, R, O>) -> Result<()>
    where
            ...
            for row in &rows {
                ...
                for &(register, ref rule) in row.registers() {
                    if !rule_cache.get(&register).map_or(false, |c| c == &rule) {
                        ...
                        self.write_register_rule(register, rule, ra)?;
                    }
                }
    ...
    fn write_cfa_rule<R: Reader>(&mut self, rule: &CfaRule<R>) -> Result<bool> {
        use gimli::CfaRule::*;
        let formatted = match rule {
            &RegisterAndOffset { register, offset } => {
                format!("{} {} +", Self::format_register(register), offset)
            }
    ...
    fn write_register_rule<R: Reader>(
        ...
        let formatted = match rule {
            ...
            &SameValue => Self::format_register(register).into(),
            &Offset(offset) => format!(".cfa {} + ^", offset),
            &ValOffset(offset) => format!(".cfa {} +", offset),
            &Register(register) => Self::format_register(register).into(),
            &Expression(_) => return Ok(false),
    ...
    fn format_register(number: u8) -> &'static str {
        // TODO(ja): match architecture...
        let names = registers::X86_64;
        names[number as usize]
    }
}
